...
from filemanager import getActiveDisplayData, copyFile
...
class ChapterSelectFrame(Frame):
    def __init__(self, parent, appconfig:dict, listBoxUpdateCommand:callable):
        ...
        self.chapterSelectSpinbox = Spinbox(self, from_=1, to=appconfig["maximumChapter"], width=2, wrap=True, textvariable=self.chapterVariable)
        self.chapterSelectSpinbox.config(command=lambda: listBoxUpdateCommand(self.chapterVariable.get()))
...
class ActiveFrame(Frame):
    ...
    def getSelectedSaveSlot(self):
        ...
        return {
            ...
            "chapter": self.currentChapter,
            "slot": selected[0],
            "isComplete": self.activeSavesData["slotData"][int(selected[0])]["isComplete"]
        }

class BackupFrame(Frame):
    def __init__(self, parent, appConfig:dict, newFolderEnabled:bool=False, hideSaves:bool=False):
        super().__init__(parent)
        self.parent = parent
        self.maxChapter = appConfig["maximumChapter"]
        ...
        self.navigateDirectoryDown = Button(self.buttonBox, text="Open", command=self.openFolder, state=DISABLED)
        self.navigateDirectoryDown.pack(side=LEFT)
        self.navigateDirectoryUp = Button(self.buttonBox, text="Back", command=self.goBack, state=DISABLED)
        self.navigateDirectoryUp.pack(side=LEFT)
        self.newFolderButton = Button(self.buttonBox, text="New Folder", command=self.createNewFolder)
    ...
    def setChapter(self, chapter):
        if not os.path.exists(os.path.join(os.environ["DSM_BKP_PATH"], f"CH{chapter}")):
            showerror(title="Fatal Error", message=f"Error, chapter {chapter} backup path does not exist")
    ...
    def populateListbox(self):
        ...
        for file in os.listdir(self.currentPath):
            if file.endswith(".drsave") and not self.hideSaves:
                ...
                self.backupListbox.insert('end', f"[Save] {file.replace('.drsave', '')}")
    ...
    def openFolder(self):
        ...
        if not os.path.exists(folderPath):
            return showerror(title="Error", message=f"Folder {selectedItem[1]} does not exist.")
    ...
    def goBack(self):
        ...
        if not os.path.exists(parentPath):
            return showerror(title="Error", message=f"Parent directory {parentPath} does not exist.")
        # Update the current path and display it
        self.currentPath = parentPath
        # Update the display path to remove the last folder
        newDisplayPath = self.displayPath.get().split("\\")[:-2] # Remove the last folder and the trailing slash
    ...
    def createNewFolder(self):
        ...
        if any(char in newFolderName for char in invalidChars):
            return showerror(title="Error", message=f"Folder name cannot contain any of the following characters: {invalidChars}")
        ...
        if newFolderName in [item[1] for item in self.backupListboxData]:
            return showerror(title="Error", message=f"Folder {newFolderName} already exists.")
        ...
        try:
            ...
            return showerror(title="Error", message=f"Failed to create folder: {str(e)}")
    ...
    def getSelectedSave(self):
        ...
        return {
            ...
            "selectedPath": os.path.join(self.currentPath, self.backupListboxData[selected[0]][1]),
        }
...
class BackupSaveLocationSelectorFrame(LabelFrame):
    def __init__(self, parent, initialDir:str, *args, entryWidth:int=40, **kwargs):
        ...
        self.saveDir = ""
        self.dirEntry = Entry(self, width=self.entryWidth, textvariable=self.dirDisplay, state=DISABLED)
    ...
    def selectBackupFolder(self):
        result = openFolderPicker("Select the backup save location: ", initialDir=self.initialDir, entryWidth=self.entryWidth)
        if not result:
            return
...
class ActiveSaveLocationSelectorFrame(LabelFrame):
    def __init__(self, parent, initialDir:str, *args, entryWidth:int=40, **kwargs):
        ...
        self.defaultLocation = Radiobutton(self, text="Default ( %localappdata%\\DELTARUNE )", variable=self.selectedOption, value=1,command=self.sel)
        ...
        self.customLocation = Radiobutton(self, text="Custom Location", variable=self.selectedOption, value=2,command=self.sel)
        self.customLocation.grid(row=2, sticky=W)

        self.entryFrame = Frame(self)
        self.dirDisplay = StringVar(value="[Nothing Selected]")
        self.saveDir = ""
        self.dirEntry = Entry(self.entryFrame, textvariable=self.dirDisplay, width=self.entryWidth)
        self.selectButton = Button(self.entryFrame, text="Select", command=self.selectActiveFolder)
        self.dirEntry.grid(column=0, row=0, padx=(5, 2.5), pady=(0,5))
        self.selectButton.grid(column=1, row=0, padx=(2.5,5), pady=(0,5))
    ...
    def selectActiveFolder(self):
        result = openFolderPicker("Select the custom active save location: ", initialDir=self.initialDir, entryWidth=self.entryWidth)
        if not result:
            return
        self.saveDir = result[0]
        self.dirDisplay.set(result[1])
...
class GameSelectSelectorFrame(LabelFrame):
    def __init__(self, parent, initialDir:str, *args, entryWidth:int=40, **kwargs):
        ...
        self.selectedOption = IntVar()
        self.defaultLocation = Radiobutton(self, text="Use Steam Link", variable=self.selectedOption, value=1,command=self.sel)
        self.defaultLocation.grid(row=1, sticky=W)
        ...
        self.exePath = ""
        self.exeEntry = Entry(self.entryFrame, textvariable=self.exeDisplay, width=self.entryWidth)
        self.selectButton = Button(self.entryFrame, text="Select", command=self.selectGameExecutable)
    ...
    def selectGameExecutable(self):
        result = openFilePicker("Select the custom game executable location: ", initialDir=self.initialDir, entryWidth=self.entryWidth, 
