...
class FirstTimeSetup(Dialog):
    ...
    def body(self, master):
        ...
        self.descriptionLabel = Label(self.mainFrame, 
                                    text="This seems to be the first time you are running this program. Please complete the steps below.",
                                    wraplength=350,
                                    justify=LEFT)
            
        
        self.backupSaveFrame = BackupSaveLocationSelectorFrame(self.mainFrame, self.initialDir, text="Backup Save Location", entryWidth=self.entryWidth)
        self.activeSaveFrame = ActiveSaveLocationSelectorFrame(self.mainFrame, self.initialDir, text="Active Save Location", entryWidth=self.entryWidth)
...
class GameSelectPopup(Dialog):
    ...
    def body(self, master):
        ...
        self.gameSelectFrame = GameSelectSelectorFrame(self.mainFrame, self.initialDir, text="Game Save Location", entryWidth=self.entryWidth)
        self.descriptionLabel.pack(side=TOP, anchor=NW, padx=5, pady=5)
    ...
    def validate(self):
        if self.gameSelectFrame.selectedOption.get() == 2:
            if not os.path.exists(self.gameSelectFrame.exePath):
                showerror(title="Error", message=f"Game exacutable location does not exist.\n{self.gameSelectFrame.exePath}")
                return False
...
class SettingsPopup(Dialog):
    ...
    def body(self, master):
        ...
        self.backupSaveFrame = BackupSaveLocationSelectorFrame(self.mainFrame, self.initialDir, text="Backup Save Location", entryWidth=self.entryWidth)
        self.activeSaveFrame = ActiveSaveLocationSelectorFrame(self.mainFrame, self.initialDir, text="Active Save Location", entryWidth=self.entryWidth)
        self.gameSelectFrame = GameSelectSelectorFrame(self.mainFrame, self.initialDir, text="Game Executable Location", entryWidth=self.entryWidth)
        self.backupSaveFrame.pack(side=TOP, anchor=NW, padx=5, pady=5)
        self.activeSaveFrame.pack(side=TOP, anchor=NW, padx=5, pady=5)
        self.gameSelectFrame.pack(side=TOP, anchor=NW, padx=5, pady=5)
        ...
        try:
            ...
            showerror(title="Error", message="User configuration file not found. Please run the application first to create it.")
            return
    ...
    def validate(self):
        ...
        if self.gameSelectFrame.selectedOption.get() == 2:
            if not os.path.exists(self.gameSelectFrame.exePath):
                showerror(title="Error", message=f"Game Executable Location does not exist.\n{self.gameSelectFrame.exePath}")
                return False
...
class BackupCreatePopup(Dialog):
    def __init__(self, parent, chapter:int, config:dict, title:str="", saveDir:str="."):
        self.FileDialogueTitle = title
    ...
    def body(self, master):
        ...
        self.descriptionLabel = Label(self.mainFrame,
                                    ...
                                    wraplength=350,
                                    justify=LEFT)
        self.backupFrame = BackupFrame(self.mainFrame, self.config, newFolderEnabled=True, hideSaves=True)
        ...
        self.backupNameEntry = Entry(self.mainFrame, textvariable=self.backupName, width=self.entryWidth)
    ...
    def validate(self):
        ...
        if os.path.exists(os.path.join(selectedPath, selectedSaveFileName)):
            showerror(title="Error", message=f"A save file with the name '{selectedSaveFileName}' already exists in the selected directory.")
            return False
