...
class DragManager:
    ...
    def _swapItems(self, sourceListbox: DraggableListbox, sourceIndex: int, targetListbox: DraggableListbox, targetIndex: int) -> None:
        # Perform the swap
    ...
    def onDrag(self, event) -> None:
        ...
        if self.drag.listbox is not None and self.dragWindow is not None and self.drag.index is not None:
        ...
        if self.drag.listbox is not None and self.drag.index is not None:
            ...
            if targetListbox is not None and targetListbox in self.listboxes:
                ...
                if (0 <= dropIndex < targetListbox.size() and
                    self.drag.id is not None and
                    targetListbox.getItemId(dropIndex) is not None):
                    
                    if self._canDrop(targetListbox, dropIndex):
                        self._swapItems(self.drag.listbox, self.drag.index, targetListbox, dropIndex)
    ...
    def _canDrop(
        ...
        if (targetListbox == self.drag.listbox and not self.drag.listbox.allowInternalSwap):
            return False
